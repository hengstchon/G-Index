...
function title(path) {
  ...
  if (model.is_search_page) {
    $("title").html(
      `${document.siteName} - ${drive_name} - Search Result for ${model.q} `
    );
  } else {
	 ...
	 <div class="mdui-row">
	  <ul class="mdui-list">
	   <li class="mdui-list-item th">
	    <div class="mdui-col-xs-8 mdui-col-sm-7">
	     File
  ...
  for (i in files) {
    ...
    if (item["mimeType"] == "application/vnd.google-apps.folder") {
      html += `<li class="mdui-list-item mdui-ripple"><a href="${p}" class="folder">
	            <div class="mdui-col-xs-8 mdui-col-sm-7 mdui-text-truncate">
...
function onSearchResultItemClick(a_ele) {
  ...
  $.post(`/${cur}:id2path`, { id: a_ele.id }, function (data) {
    ...
    dialog = mdui.dialog({
      title:
        '<i class="mdui-icon material-icons">&#xe811;</i>Failed to get the target path',
      content:
